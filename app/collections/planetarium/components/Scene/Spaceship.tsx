...
import { Zap, Compass, Target, RefreshCw, Gauge, ArrowRightCircle, Plane, AlertCircle, Fuel, Crosshair, AlertTriangle, Lock, Wallet, Navigation } from 'lucide-react';
...
export function SpaceshipController({ active, lockedTargetId, hoveredTargetId }: { active: boolean, lockedTargetId: string | null, hoveredTargetId: string | null }) {
    ...
    const { timeRef, fuel: contextFuel, updateFuel, boost: contextBoost, updateBoost, saveGame, savedPosition, dockedAt, lastDockedNode, lastDockVector, findBody, isLoadingSave, currentShip, activeJob } = useSimulation(); 
    ...
    useFrame((state, delta) => {
        ...
        if (ap && activeJob) {
            ...
            if (targetBody) {
                ...
                // Speed Control: Target 50% of Normal Max (FA ON max is Accel/2)
                // So target = (Accel / 2) * 0.5 = Accel / 4.
                ...
                if (fuelRef.current > 0) {
                    if (currentSpeed < targetApSpeed) {
                        ...
                        // Brake if overspeed (artificial drag for cruise control)
...
export function SpaceshipHUD({ active }: { active: boolean }) {
    const { setDockedAt, generateJobsForLocation, updateFuel, updateBoost, findBody, saveGame, credits, currentShip, activeJob } = useSimulation();
    ...
    const hud = currentShip.hud || { primary: '#DFFF00', secondary: '#ffffff', alert: '#ef4444', shape: 'rounded', fontParams: '' };
    const shapeClass = hud.shape === 'angular' ? 'rounded-none clip-path-polygon-[10%_0,100%_0,100%_100%,0_100%,0_20%]' : hud.shape === 'blocky' ? 'rounded-sm' : 'rounded-t-3xl';
    const btnShape = hud.shape === 'angular' ? 'rounded-none' : hud.shape === 'blocky' ? 'rounded-sm' : 'rounded-l-full rounded-tr-lg';
