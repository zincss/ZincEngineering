...
export function StarBackground() {
    ...
    return (
        <group>
            ...
            <Stars radius={300} depth={500} count={2000} factor={4} saturation={0} fade speed={0.5} />
        </group>
    );
}

// --- INTERSTELLAR-STYLE BLACK HOLE SHADER (Realistic Physics Simulation) ---
const AccretionDiskShader = {
    uniforms: {
        time: { value: 0 },
        // NASA / Interstellar Colors: Reddish-Orange to White
        colorInner: { value: new THREE.Color("#ffddaa") }, // Hot inner edge
        colorMid: { value: new THREE.Color("#ff5500") },   // Main disk
        colorOuter: { value: new THREE.Color("#330000") }, // Cool outer edge
    },
    vertexShader: `
        varying vec2 vUv;
        varying vec3 vPos;
    ...
    fragmentShader: `
        ...
        // Noise function for gas turbulence
        ...
        void main() {
            ...
            // 1. Relativistic Doppler Beaming (Blue/Red shift approximation)
            // Left side (approaching) is brighter, Right side (receding) is dimmer
            float doppler = 1.0 + 0.6 * sin(theta + 1.5); // Offset to align with spin

            // 2. Turbulence / Swirl
            // Inner parts move faster (Keplerian velocity)
            float speed = 2.0 / (r + 0.1); 
            float noiseVal = noise(vec2(r * 10.0 - time * 0.5, theta * 4.0 + time * speed));
            
            // 3. Color Gradient (Temperature)
            // Hotter near center, cooler near edge
            vec3 baseColor = mix(colorMid, colorOuter, smoothstep(0.4, 1.0, r));
            baseColor = mix(colorInner, baseColor, smoothstep(0.2, 0.4, r));
            
            // 4. Alpha/Gap (ISCO - Inner Most Stable Circular Orbit)
            // Hard cutoff at inner edge (Event Horizon gap)
            float alpha = smoothstep(0.32, 0.35, r) * smoothstep(1.0, 0.8, r);
            
            // Composition
            vec3 finalColor = baseColor * (0.8 + 1.2 * noiseVal); // Add noise detail
            finalColor *= doppler; // Apply doppler beaming
            
            // Brightness boost for HDR bloom
...
function BlackHole({ data, onClick }: any) {
    ...
    useFrame(({ clock }) => {
        ...
        if (lensMat.current) lensMat.current.uniforms.time.value = t + 50.0; // Offset noise
    });

    return (
        <group>
            {/* 1. THE SHADOW (Event Horizon) - Pure Black */}
            <mesh 
                ref={meshRef}
            ...
            {/* 2. THE ACCRETION DISK (Flat Plane) */}
            ...
            {/* 3. THE "LENSED" DISK (Vertical warp) */}
            {/* Simulates the light from the back of the disk bending over the top/bottom */}
            ...
            {/* 4. THE PHOTON RING (Thin, bright shell) */}
            <mesh raycast={() => null}>
...
// --- NEW: VOLUMETRIC SATURN RINGS ---
// Updated to separate into Inner/Outer bands with a gap (Cassini Division)
function SaturnRings({ innerRadius, outerRadius }: { innerRadius: number, outerRadius: number }) {
    ...
    useLayoutEffect(() => {
        ...
        for (let i = 0; i < count; i++) {
            ...
            // CASSINI DIVISION LOGIC
            // Define bands (Assuming Input: Inner=12, Outer=24)
            // Ring B (Inner, Dense): 12.0 -> 17.5
            // Cassini Gap: 17.5 -> 19.5 (Empty)
            // Ring A (Outer, Sparse): 19.5 -> 24.0
            
            let r = 0;
            const seed = Math.random();
            const ringB_Width = 5.5; 
            const ringA_Width = 4.5;
            // Weigh probability by area (outer rings have more area) roughly
            // But Ring B is denser visibly.
            
            if (seed > 0.4) {
                 // 60% chance for Inner Ring B (Denser look)
                 r = innerRadius + Math.random() * ringB_Width;
            } else {
                 // 40% chance for Outer Ring A
            ...
            // Very thin vertical spread for "Disc" look
            ...
            // Varied Scale
            // Inner ring particles slightly larger/clumpier
            ...
            // Color Logic: Inner ring slightly warmer/brighter
...
export function Planet({ data, isSelected, onClick, onSelectRef, isCinematic, showOrbits, showLabels }: any) {
    ...
    // Only load texture ring if it exists AND it's NOT Saturn (since we use volumetric for Saturn now)
        ...
        <group>
            ...
            <group ref={groupRef}>
                <group rotation={[0, 0, tiltRadians]}>
                    ...
                    {/* Volumetric Rings for Saturn - FIXED ORIENTATION */}
                    {data.id === 'saturn' && (
                        // Removed the 90deg X rotation. Now it sits flat relative to the planet's tilt.
                        <group>
                             <SaturnRings innerRadius={13} outerRadius={25} />
                        </group>
                    )}

                    {/* Texture Rings for others (if any) */}
