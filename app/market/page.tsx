...
export default function MarketHubPage() {
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-white selection:bg-[#DFFF00] selection:text-black flex flex-col relative overflow-hidden">
      ...
      {/* HEADER SECTION */}
      <div className="pt-24 pb-8 px-4 md:pt-32 md:pb-8 md:px-6 max-w-[1800px] mx-auto border-b border-zinc-800 w-full">
        ...
        {/* TABS NAVIGATION */}
        <div className="flex gap-2 md:gap-8 overflow-x-auto pb-1 no-scrollbar mask-gradient-right">
            <TabButton 
...
function TabButton({ active, onClick, icon, label }: { active: boolean, onClick: () => void, icon: any, label: string }) {
    return (
        <button 
            onClick={onClick}
            className={`
                group flex items-center gap-2 px-6 py-4 text-xs font-black uppercase tracking-widest transition-all whitespace-nowrap rounded-t-xl border-b-2
                ${active 
