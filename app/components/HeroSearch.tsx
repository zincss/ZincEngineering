...
// FIX: Using relative path (../../) to go up 2 levels to the root 'lib' folder
...
export default function HeroSearch() {
  ...
  useEffect(() => {
    const initSystem = async () => {
      // Fetch 10 Warframes just to warm up the connection
  ...
  return (
    <div className="relative w-full max-w-3xl mx-auto z-50" ref={dropdownRef}>
      ...
      <div className="relative group">
        {/* Status Indicator */}
        <div className="absolute top-0 left-0 -translate-y-full flex items-center gap-2 mb-1">
        ...
        <div className="absolute inset-y-0 left-0 pl-6 flex items-center pointer-events-none">
          <Search size={24} className={systemReady ? "text-black" : "text-zinc-300"} strokeWidth={3} />
        </div>
        
        <input
          type="text"
          className="w-full bg-white border-2 border-black rounded-none py-6 pl-16 pr-4 text-2xl font-bold tracking-tight text-black placeholder:text-zinc-300 focus:outline-none focus:ring-0 focus:bg-zinc-50 transition-all uppercase"
          placeholder={systemReady ? "ENTER SUBJECT NAME..." : "INITIALIZING DATABASE..."}
          value={query}
        ...
        <div className={`absolute bottom-0 right-0 w-4 h-4 border-t-2 border-l-2 border-black transition-colors ${systemReady ? 'bg-acid' : 'bg-zinc-300'}`} />
      </div>

      {/* Results Dropdown */}
      {showDropdown && results.length > 0 && (
        <div className="absolute top-full left-0 right-0 mt-4 bg-white border-2 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] animate-in fade-in slide-in-from-top-2 z-50">
          ...
          {results.map((item) => (
            <div
              ...
              className="group flex items-center justify-between p-4 cursor-pointer hover:bg-acid transition-colors border-b border-zinc-200 last:border-0"
            >
              <div className="flex items-center gap-6">
                <div className="h-12 w-12 border border-black bg-zinc-50 p-1 relative flex items-center justify-center overflow-hidden">
                   <img 
                ...
                <div>
                  <h4 className="text-xl font-black uppercase leading-none group-hover:text-black">{item.name}</h4>
