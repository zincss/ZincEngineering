...
import { fetchSchedule } from '@/app/sports/nfl/lib/espn';
import { ArrowLeft, Zap, Calendar } from 'lucide-react';

export const dynamic = 'force-dynamic';

export default async function NFLBreakdownSchedule() {
    ...
    <main className="min-h-screen bg-zinc-950 text-white p-6 md:p-12 pb-24">
       ...
       <header className="mb-12">
          <h1 className="text-5xl md:text-7xl font-black uppercase tracking-tighter text-white mb-2">
             NFL <span className="text-zinc-800 text-stroke-white">Schedule</span>
          </h1>
       ...
       <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {games.map((game: any) => (
             <Link key={game.id} href={`/sports/breakdown/nfl/${game.id}`} className="group relative bg-zinc-900/30 border border-white/5 hover:border-[#DFFF00] p-6 rounded-3xl transition-all duration-300 hover:bg-zinc-900/50">
