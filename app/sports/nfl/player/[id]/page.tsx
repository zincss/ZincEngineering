...
export default function NFLPlayerPage({ params }: { params: { id: string } }) {
    ...
    <div className="min-h-screen bg-black pb-20 text-white selection:bg-[#DFFF00] selection:text-black">
        ...
        <div className="max-w-6xl mx-auto px-6 py-12 grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="space-y-6">
                ...
                <div className="bg-zinc-900/30 border border-zinc-800 p-6">
                    <h3 className="text-sm font-black uppercase text-[#DFFF00] mb-6 flex items-center gap-2">
                        <BarChart3 size={16} /> Annual Stats
                    </h3>
                     {data.stats && data.stats.length > 0 ? (
                        <div className="grid grid-cols-2 gap-4">
                            {data.stats.slice(0, 6).map((stat: any, i: number) => (
                                <div key={i} className="bg-black border border-zinc-800 p-3 text-center">
                                    ...
                                    <div className="text-lg font-mono font-bold text-white">{stat.displayValue}</div>
                                </div>
                            ))}
                        </div>
                    ) : (
            ...
            <div className="lg:col-span-2">
                <div className="bg-zinc-900/30 border border-zinc-800 h-full">
                    <div className="p-6 border-b border-zinc-800 flex justify-between items-center">
                        ...
                        <span className="text-[10px] font-mono font-bold text-zinc-500 uppercase tracking-widest">Game Log</span>
                    </div>
                        ...
                        <table className="w-full text-left font-mono text-xs">
                            <thead>
                                <tr className="bg-black text-zinc-500 border-b border-zinc-800">
                                    ...
                                    <th className="p-4 font-black tracking-widest text-right">STATS</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-zinc-800/50">
                                {logs && logs.length > 0 ? logs.map((game, i) => (
                                    <tr key={i} className="hover:bg-zinc-900/50 transition-colors">
                                        ...
                                        <td className="p-4 text-right">
                                            <div className="flex justify-end gap-2 flex-wrap">
                                                {/* Safely map simple stats */}
                                                {Array.isArray(game.stats) && game.stats.slice(0, 5).map((s: string | number, j: number) => (
                                                    <span key={j} className="bg-zinc-800 px-2 py-1 rounded text-[10px] text-zinc-300 min-w-[24px] text-center">
                                                        {s}
                                                    </span>
                                                ))}
                                            </div>
                                    ...
                                    <tr>
                                        <td colSpan={4} className="p-12 text-center text-zinc-600 font-mono">
